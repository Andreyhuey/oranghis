...
router.get("/:id", async (request, response) => {
  try {
    ...
    const book = await CS.findById(id);
...
router.put("/:id", async (request, response) => {
  try {
    ...
    const result = await CS.findByIdAndUpdate(id, request.body);
...
router.get("/search/:query", async (request, response) => {
  try {
    ...
    const book = await CS.find({});

    const searchQuery = book.filter(
      (item) =>
